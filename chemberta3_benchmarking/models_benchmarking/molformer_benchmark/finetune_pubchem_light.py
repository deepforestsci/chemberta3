...
class LightningModule(pl.LightningModule):
    ...
    def __init__(self, config, tokenizer):
        ...
        # Updated to use `self.save_hyperparameters(config)` instead of directly assigning `self.hparams = config`,
        # which is no longer allowed in newer versions of PyTorch Lightning (see discussion #7525). This method
        # properly registers the hyperparameters, making them accessible via `self.hparams`, and ensures they are
        # saved with checkpoints for reproducibility and logging.
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        try:
            ...
            print(f"Error loading RNG states from checkpoint: {e}")
    ...
    def validation_epoch_end(self, outputs):
        ...
        for dataset_idx, batch_outputs in enumerate(outputs):
            ...
            # calculates RMSE and MAE scores
            rmse = np.sqrt(mean_squared_error(actuals_cpu, preds_cpu))
            mae = np.mean(np.abs(actuals_cpu - preds_cpu))
            print(dataset, " r2: ", r2)
            print(dataset, " rmse: ", rmse)
            print(dataset, " pearson_r: ", pearson_r)
            ...
            tensorboard_logs.update(
                {
                    ...
                    self.hparams.measure_name + "_" + dataset + "_rmse": rmse,
                    self.hparams.measure_name + "_" + dataset + "_mae": mae,
                }
            )
        ...
        if (
            ...
            self.min_loss[self.hparams.measure_name + 'min_valid_rmse'] = tensorboard_logs[
                self.hparams.measure_name + "_valid_rmse"
            ]
            self.min_loss[self.hparams.measure_name + 'min_test_rmse'] = tensorboard_logs[
                self.hparams.measure_name + "_test_rmse"
            ]

            self.min_loss[self.hparams.measure_name + 'min_valid_mae'] = tensorboard_logs[
                self.hparams.measure_name + "_valid_mae"
            ]
            self.min_loss[self.hparams.measure_name + 'min_test_mae'] = tensorboard_logs[
                self.hparams.measure_name + "_test_mae"
            ]
        ...
        append_to_file(
            ...
            f"{self.min_loss[self.hparams.measure_name + 'min_valid_rmse']},"
            f"{self.min_loss[self.hparams.measure_name + 'min_test_rmse']},"
            f"{self.min_loss[self.hparams.measure_name + 'min_valid_mae']},"
            f"{self.min_loss[self.hparams.measure_name + 'min_test_mae']}"
        )
...
def main():
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(every_n_epochs=1, save_last=True, dirpath=checkpoint_dir, filename='checkpoint', verbose=True)
    ...
    if os.path.isfile(last_checkpoint_file):
        ...
        print(f"not resuming from any previous finetuning checkpoint")
