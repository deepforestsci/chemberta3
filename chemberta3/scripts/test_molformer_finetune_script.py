...
from molformer_finetune_script import load_dataset, get_tuning_utils, download_pretrained_model_from_S3, modify_model_keys, hyperparam_search, evaluate_model, main, filter_datasets
import boto3
import torch
import tempfile
import os
...
def test_get_tuning_utils():
    ...
    datasets = ['delaney', 'bace_regression', 'clearance', 'lipo']
    ...
    datasets = ['clintox', 'bace_classification', 'bbbp', 'hiv', 'sider', 'tox21']
    for dataset in datasets:
        output = get_tuning_utils(dataset)
...
def test_modify_model_keys_and_download_from_S3():
    ...
    file_key = 'MoLFormer-pretrained-models/Molformer-550M-2epochs/checkpoint_001737/model_ckpt'
...
def test_hyperparam_search_and_evaluate_model():
    ...
    temp_dir = tempfile.TemporaryDirectory()
    model = MoLFormer(n_tasks=1, task='mlm', model_dir=temp_dir.name)
...
def test_main():
    ...
    bucket_name = 'chemberta3'
    file_key = 'MoLFormer-pretrained-models/Molformer-550M-2epochs/checkpoint_001737/model_ckpt/checkpoint1.pt'
